...
export default async function decorate(block) {
  ...
  const navigationItems = itemRows.filter((row) => row.children.length === 4 && row.querySelector('ul'));
  const pressReleaseItems = itemRows.filter((row) => row.children.length === 4 && !row.querySelector('ul'));
  ...
  navigationItems.forEach((row) => {
    ...
    const megaMenuContent = megaMenuCell.innerHTML.trim();
    if (megaMenuContent) {
  ...
  iconNavItems.forEach((row) => {
    ...
    if (labelText === 'contact us') {
      li.classList.add('mail');
    } else if (labelText === 'search') {
      li.classList.add('search');
    } else {
      li.classList.add('mail'); // Default to 'mail' if not recognized
    }
  ...
  iconNavItems.forEach((row) => {
    ...
    // For desktop, original HTML has SVG for mail/search, not text.
    const labelText = labelCell.textContent.trim().toLowerCase();
    if (labelText === 'contact us') {
      li.classList.add('mail');
      ...
      const swiperEl = searchResultBox.querySelector('.swiper.scrollSwiper');
  ...
  const logo80Link = document.createElement('a');
  const logo80Anchor = logo80LinkRow.querySelector('a');
  if (logo80Anchor) {
    logo80Link.href = logo80Anchor.href;
  }
  const logo80Picture = logo80Row.querySelector('picture');
  if (logo80Picture) {
    const logo80Img = logo80Picture.querySelector('img');
    const optimizedLogo80 = createOptimizedPicture(logo80Img.src, logo80Img.alt, false, [{ width: '74' }]);
    moveInstrumentation(logo80Img, optimizedLogo80.querySelector('img'));
    logo80Link.append(optimizedLogo80);
  }
  moveInstrumentation(logo80Row, logo80Link);
  moveInstrumentation(logo80LinkRow, logo80Link);
  logo80Div.append(logo80Link);
