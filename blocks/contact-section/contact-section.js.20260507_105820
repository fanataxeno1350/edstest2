...
export default async function decorate(block) {
  ...
  locationDetail.style.paddingLeft = '19px';
  locationDetail.textContent = locationDetailRow.textContent.trim();
  moveInstrumentation(locationDetailRow, locationDetail);
  locationWrapper.append(locationDetail);
  ...
  if (foundEmailLink) {
    ...
    // Display the actual email address, not the JCR path
  ...
  socialLinkRows.forEach((row) => {
    // For social link item rows, the model specifies a single 'link' field
    ...
    // Determine which SVG icon to use based on the URL
  ...
  const foundMapLink = mapEmbedRow.querySelector('a');
  if (foundMapLink) {
    mapEmbedIframe.src = foundMapLink.href;
  }
  mapEmbedIframe.setAttribute('width', '100%');
  mapEmbedIframe.setAttribute('style', 'border:0;');
  mapEmbedIframe.setAttribute('allowfullscreen', '');
  mapEmbedIframe.setAttribute('loading', 'lazy');
  mapEmbedIframe.setAttribute('referrerpolicy', 'no-referrer-when-downgrade');
  moveInstrumentation(mapEmbedRow, mapEmbedIframe);
  contactDetailsCol.append(mapEmbedIframe);
