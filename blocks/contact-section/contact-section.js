...
export default async function decorate(block) {
  ...
  const [
    ...
    workLocationRow,
    emailLinkRow,
    socialLinksContainerRow, // Placeholder for container field, actual items are in itemRows
    externalEmbedsContainerRow, // Placeholder for container field, actual items are in itemRows
  ...
  // Filter itemRows into their respective sub-component types
  ...
  moveInstrumentation(sectionHeadingRow.children[0], sectionHeading); // Move instrumentation from the cell
  sectionHeading.textContent = sectionHeadingRow.children[0].textContent.trim();
  contactContainerDiv.append(sectionHeading);

  const sectionSubheading = document.createElement('h3');
  moveInstrumentation(sectionSubheadingRow.children[0], sectionSubheading); // Move instrumentation from the cell
  sectionSubheading.textContent = sectionSubheadingRow.children[0].textContent.trim();
  contactContainerDiv.append(sectionSubheading);
  ...
  workLocationP.style.paddingLeft = '19px'; // Hardcoded style from original HTML
  moveInstrumentation(workLocationRow.children[0], workLocationP); // Move instrumentation from the cell
  workLocationP.textContent = workLocationRow.children[0].textContent.trim();
  workLocationDiv.append(workLocationP);
  contactDetailsCol.append(workLocationDiv);
  ...
  const emailLinkCell = emailLinkRow.children[0]; // Access the cell
  const foundEmailLink = emailLinkCell.querySelector('a');
  if (foundEmailLink) {
    emailLink.href = foundEmailLink.href;
    // The text content should be the actual email address, not the JCR path
    emailLink.textContent = foundEmailLink.textContent.trim().replace('/content/site/', ''); // Adjust based on actual content
    if (emailLink.textContent.startsWith('mailto:')) {
      emailLink.textContent = emailLink.textContent.replace('mailto:', '');
    }
  }
  moveInstrumentation(emailLinkCell, emailLink); // Move instrumentation from the cell
  ...
  socialLinkRows.forEach((row) => {
    const [socialLinkCell] = [...row.children]; // Destructure for fixed schema
    ...
    moveInstrumentation(socialLinkCell, socialLinkAnchor); // Move instrumentation from the cell
  ...
  externalEmbedRows.forEach(async (row) => {
    ...
    if (embedKind === 'elfsight-widget') {
    ...
    } else if (embedKind === 'twitter-embed' || embedKind === 'instagram-embed' || embedKind === 'tiktok-embed') {
      const platforms = {
    ...
    } else if (embedUrl && embedUrl.includes('google.com/maps/embed')) {
      ...
      // Fallback for unknown embeds or just display the URL
      const link = document.createElement('a');
      link.href = embedUrl;
  ...
  hiddenInput.setAttribute('value', ''); // Value from original HTML
  // No instrumentation to move for a hardcoded hidden input
  ...
  moveInstrumentation(formNamePlaceholderRow.children[0], nameInput); // Move instrumentation from the cell
  ...
  moveInstrumentation(formEmailPlaceholderRow.children[0], emailInput); // Move instrumentation from the cell
  ...
  moveInstrumentation(formMessagePlaceholderRow.children[0], messageTextarea); // Move instrumentation from the cell
  ...
  moveInstrumentation(formButtonLabelRow.children[0], submitButton); // Move instrumentation from the cell
  contactForm.append(submitButton);

  formCol.append(contactForm);
  rowDiv.append(formCol);
  contactContainerDiv.append(rowDiv);
  containerDiv.append(contactContainerDiv);
