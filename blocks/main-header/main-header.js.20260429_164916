...
export default function decorate(block) {
  ...
  navigationItems.forEach((row) => {
    ...
    svgSpan.innerHTML = `<svg viewBox="-23.5 -23.5 122.80 122.80" fill="#000000" stroke="#000000" stroke-width="4.851456000000001">
    ...
    if (hierarchyRoot) {
      ...
      leftDiv.classList.add('left-div'); // Corrected classList.classList.add
      const heading = document.createElement('h4');
      heading.classList.add('left-div-heading');
      const headingLink = document.createElement('a');
      headingLink.textContent = rootEl.textContent;
  ...
  mobileIconLi.append(contactUsLinkMobile);
  ...
  const createSuggestionSection = (label, keywords) => {
    ...
    section.classList.add('search-suggestions-wrap');
    const labelDiv = document.createElement('div');
    labelDiv.classList.add('label');
    labelDiv.textContent = label;
    section.append(labelDiv);
    ...
    section.append(tokensWrap);
    return section;
  };

  searchWrapContent.append(createSuggestionSection('Popular Keywords:', popularKeywords));
  searchWrapContent.append(createSuggestionSection('Recommended for you:', recommendedKeywords));

  // Toggle search screen
  const toggleSearch = () => {
    searchScreenWrap.classList.toggle('active');
    searchLinkMobile.querySelector('.lens').classList.toggle('active');
    searchLinkMobile.querySelector('.close').classList.toggle('active');
    searchLinkDesktop.querySelector('.lens').classList.toggle('active');
    searchLinkDesktop.querySelector('.close').classList.toggle('active');
  ...
  wrap.append(searchScreenWrap);

  // 80th Year Logo
  const year80LogoDiv = document.createElement('div');
  year80LogoDiv.classList.add('logo', 'year-80-logo');
  const year80LogoLink = document.createElement('a');
  const found80YearLogoLink = logo80YearsLinkRow.querySelector('a');
  if (found80YearLogoLink) {
    year80LogoLink.href = found80YearLogoLink.href;
  }
  const year80LogoPicture = logo80YearsRow.querySelector('picture');
  if (year80LogoPicture) {
    ...
    optimizedImg.setAttribute('width', '74');
    optimizedImg.setAttribute('height', '60');
    year80LogoLink.append(optimizedPic);
  }
  moveInstrumentation(logo80YearsRow, year80LogoLink);
