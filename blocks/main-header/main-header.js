...
function transformNestedLists(rootUl) {
  ...
  // Add classes to <a> elements within the hierarchy
  rootUl.querySelectorAll('a').forEach((a) => {
    a.classList.add('nav-menu-item'); // Assuming 'nav-menu-item' is a relevant class
  });
}
...
export default function decorate(block) {
  ...
  block.innerHTML = '';
  block.classList.add('main-header'); // Class from ORIGINAL HTML
  ...
  wrap.classList.add('wrap'); // Class from ORIGINAL HTML
  ...
  hamburger.classList.add('hamburger'); // Class from ORIGINAL HTML
  ...
  mainNav.classList.add('main-nav'); // Class from ORIGINAL HTML
  ...
  desktopIconNav.classList.add('icon-nav', 'desktop-menus-icon'); // Classes from ORIGINAL HTML
  ...
  anniversaryLogoDiv.classList.add('logo', 'year-80-logo'); // Classes from ORIGINAL HTML
  ...
  navigationItems.forEach((row) => {
    // Correctly destructure cells for navigation-item
    ...
    moveInstrumentation(labelCell, rootEl); // Move instrumentation from label cell to rootEl
    ...
    if (hierarchyRoot) {
      ...
      megaMenu.classList.add('mega-menu'); // Class from ORIGINAL HTML
      const megaMenuWrap = document.createElement('div');
      megaMenuWrap.classList.add('wrap', 'container'); // Classes from ORIGINAL HTML
      const centerDiv = document.createElement('div');
      centerDiv.classList.add('center-div'); // Class from ORIGINAL HTML
      const subNavWrap = document.createElement('div');
      subNavWrap.classList.add('sub-nav-wrap'); // Class from ORIGINAL HTML

      // Apply instrumentation and classes to the hierarchyRoot and its children
      moveInstrumentation(hierarchyCell, hierarchyRoot);
      hierarchyRoot.querySelectorAll('ul').forEach(ul => ul.classList.add('sub-menu')); // Example class
      hierarchyRoot.querySelectorAll('li').forEach(liItem => liItem.classList.add('list-item')); // Example class
      hierarchyRoot.querySelectorAll('a').forEach(a => a.classList.add('nav-menu-item')); // Example class
  ...
  mobileIconNav.classList.add('icon-nav', 'mobile-menus-icon'); // Classes from ORIGINAL HTML
  ...
  mailLi.classList.add('mail'); // Class from ORIGINAL HTML
  const mailLink = document.createElement('a');
  mailLink.href = 'https://www.mahindra.com/contact-us';
  mailLink.innerHTML = `<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
  ...
  mobileIconUl.append(mailLi.cloneNode(true)); // For mobile
  desktopIconUl.append(mailLi); // For desktop

  // Search icon and functionality
  const searchLi = document.createElement('li');
  searchLi.classList.add('search'); // Class from ORIGINAL HTML
  const searchLink = document.createElement('a');
  searchLink.href = '#';
  searchLink.innerHTML = `<svg viewBox="0 0 21 21" fill="none" class="lens">
  ...
  searchScreenWrap.classList.add('search-screen-wrap'); // Class from ORIGINAL HTML
  const searchWrapInner = document.createElement('div');
  searchWrapInner.classList.add('wrap'); // Class from ORIGINAL HTML
  ...
  searchInputWrap.classList.add('search-wrap'); // Class from ORIGINAL HTML
  searchInputWrap.innerHTML = `<div class="search-icon">
  ...
  searchLink.addEventListener('click', (e) => {
    e.preventDefault();
    searchLi.classList.toggle('active'); // Class from ORIGINAL HTML
    searchScreenWrap.classList.toggle('active'); // Class from ORIGINAL HTML
  });

  mobileIconUl.append(searchLi.cloneNode(true)); // For mobile
  desktopIconUl.append(searchLi); // For desktop

  // Hamburger menu toggle
  hamburger.addEventListener('click', () => {
    mainNav.classList.toggle('active'); // Class from ORIGINAL HTML
    hamburger.classList.toggle('active'); // Class from ORIGINAL HTML
    block.classList.toggle('active'); // Class from ORIGINAL HTML
  });
  ...
  window.addEventListener('scroll', () => {
    if (window.scrollY > lastScrollY && window.scrollY > 100) {
      block.classList.add('nav-up'); // Class from ORIGINAL HTML
    } else {
      block.classList.remove('nav-up'); // Class from ORIGINAL HTML
    }
